    ...
    <div className='w-3xs flex flex-col'>
        <div className='relative'>
            <div className='w-full h-full absolute top-0 left-0 rounded-2xl transition-all bg-black opacity-0 flex justify-center items-center hover:opacity-60'>
                <Link href={`/movies/${movie.id}`} className="cursor-pointer"><FaCirclePlay color='red' size={45} /></Link>
            </div>

            <Image src={img_url+movie.poster_path} alt='' width={100} height={200} className='w-full rounded-2xl object-contain' />
        </div>
