...
import { IMovie, ProductionCompanie } from "@/app/components/movie-item";
...
export default function MoviePage() {
  ...
  return (
    <div className="w-full flex flex-col gap-10">
      <div
        className="w-full flex gap-10 p-10 relative bg-cover"
        style={{
      ...
      >
        ...
        <div className="flex flex-col gap-3 p-5 backdrop-blur-lg rounded-2xl">
            ...
            <ul className="flex flex-wrap gap-4">
              {
                movie.genres.map((genre:IGenre) => {
                  return <li key={genre.id} className="text-gray-400">{genre.name}</li>
                })
              }
            </ul>
          </div>
          ...
          {movie.production_companies?.map((companie: ProductionCompanie) => {
            return (
              <SwiperSlide key={"companie-" + companie.id}>
                <div className="w-48 h-48 flex flex-col items-center justify-center gap-3.5">
                  {companie.logo_path ? (
                    <Image
      ...
      >
          {
            actors?.map(actor => {
              return <SwiperSlide key={"actor-"+actor.id}><ActorCard actor={actor} /></SwiperSlide>
            })
          }
      </Swiper>
